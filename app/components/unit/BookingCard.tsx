...
export default function BookingCard({ unit }: { unit: Unit }) {
  ...
  const t = useUi();
  const { billingLabel, computeEstimate, dayLabel, fxRateNote, pretty } = useDates();
  ...
  const note = hasDates ? t.dateRangeNote(pretty(start), pretty(end)) : undefined;
      ...
      {againFrom !== null ? (
        ...
        <div aria-live="polite" className="mt-3">
          ...
          <p className="mt-1.5 text-sm text-copy">
            {t.nightsCount(est.nights)}, {billingLabel(est.nights)} ·{" "}
        ...
        <p className="mt-2.5 text-[15px] text-copy">{t.pickDatesForTotal}</p>
      )}
      ...
      <div className="mt-4 grid gap-2">
        {hasDates && free ? (
          ...
          <button type="button" onClick={openPicker} className={`${PRIMARY} bg-ink hover:bg-ink-hover`}>
            ...
            {hasDates ? t.chooseOtherDates : t.checkDates}
          </button>
        )}
        <a href={whatsappHref(content, { unit, note })} target="_blank" rel="noopener noreferrer" className={LINE_BTN}>
          ...
          {t.askHenrikOnWhatsapp}
        </a>
      </div>
      <p className="mt-2.5 text-center text-sm text-copy">{t.holdIsFree}</p>
    </div>
  );
}
